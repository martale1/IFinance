...
class dbTickersDatabackup:
    def __init__(self, db_path="/home/developer/PycharmProjects/learningProjects/IFinance/sotck_data4.db"):
        ...
        if result:
            ...
            data = {
                ...
                'signals': [{
                    ...
                    'MACD_Prev': result[18],
                    'MACD_Signal_Prev': result[19], 'RSI': result[20], 'SAR': result[21], 'ADX': result[22],
                    ...
                    'Bollinger_Lower': result[36], 'Date': result[37], 'TrendIndicator1': result[38],
                    'TrendIndicator2': result[39], 'TrendIndicator3': result[40], 'TrendIndicator4': result[41],
                    'Scoring': result[42],  # Aggiunta del campo Scoring
                    'Signal1': result[43], 'Signal2': result[44], 'Signal3': result[45], 'Signal4': result[46],
                    'Signal5': result[47], 'Signal6': result[48], 'CDL_Patterns': result[49]
                }]
            }
            return data
        else:
    ...
    def get_signal_counts_and_ticker_lists(self, nation, instrument_type):
        ...
        df_tickers_1 = pd.DataFrame({
            ...
            'S2': ', '.join(df[df['S2'] == 1]['Ticker']),
            'S3': ', '.join(df[df['S3'] == 1]['Ticker']),
        ...
        df_ticker_names_1 = pd.DataFrame({
            ...
            'S2': ', '.join(df[df['S2'] == 1]['Ticker_Name']),
            'S3': ', '.join(df[df['S3'] == 1]['Ticker_Name']),
        ...
        df_volumes_1 = pd.DataFrame({
            ...
            'S2': ', '.join(df[df['S2'] == 1]['Volume'].astype(str)),
            'S3': ', '.join(df[df['S3'] == 1]['Volume'].astype(str)),
        ...
        df_percent_change_2_1 = pd.DataFrame({
            ...
            'S2': ', '.join(df[df['S2'] == 1]['Percent_Change_2'].astype(str)),
            'S3': ', '.join(df[df['S3'] == 1]['Percent_Change_2'].astype(str)),
        ...
        df_signal6_1 = pd.DataFrame({
            ...
            'S2': ', '.join(df[df['S2'] == 1]['Signal6']),
            'S3': ', '.join(df[df['S3'] == 1]['Signal6']),
        ...
        df_scoring_1 = pd.DataFrame({
            ...
            'S2': ', '.join(df[df['S2'] == 1]['Scoring'].astype(str)),
            'S3': ', '.join(df[df['S3'] == 1]['Scoring'].astype(str)),
    ...
    def save_json_data(self, json_data_str, nation, instrument_type):
        ...
        fields = [
            ...
            'ATR', 'ATR_Trailing_Stop_Loss', 'Bollinger_Upper', 'Bollinger_Middle', 'Bollinger_Lower', 'Date',
        ...
        columns = [
            ...
            'date', 'trend_indicator1', 'trend_indicator2', 'trend_indicator3', 'trend_indicator4', 'scoring',
        ...
        values = [
            ...
            signals['Close'], signals['Adj Close'], signals['Volume'], float(signals['Percent_Change_2']),
            signals['Percent_Change_5'], signals['Percent_Change_10'], signals['Percent_Change_30'],
            signals['EMA_50'], signals['MACD'], signals['MACD_Signal'], signals['MACD_Histogram'], signals['MACD_Prev'],
            ...
            signals['Bollinger_Lower'], signals['Date'], signals['TrendIndicator1'], signals['TrendIndicator2'],
...
'''
myDb=dbTickersData()
ticker_data = myDb.get_sorted_tickers_by_keys("IT", 'ETC', "percent_change_2", "volume",
...
d=ticker_data.sort_values(by=['percent_change_2'],ascending=False)
print(d.to_string())
...
db = dbTickersDatabackup()
nation="IT"
instrument_type="ETF"

ts = GenerateScoringAndSignals('3LTS.MI', '3LTS.MI', ema_period=50, period='2y', candles=0)
json_data=ts.download_json()
print(json_data)
db.save_json_data(json_data, nation, instrument_type)

ts = GenerateScoringAndSignals('SBA3.MI', 'SBA3.MI', ema_period=50, period='2y', candles=0)
json_data=ts.download_json()
print(json_data)
db.save_json_data(json_data, nation, instrument_type)

ts = GenerateScoringAndSignals('3HCS.MI', '3HCS.MI', ema_period=50, period='2y', candles=0)
json_data=ts.download_json()
print(json_data)
db.save_json_data(json_data, nation, instrument_type)

ts = GenerateScoringAndSignals('3CON.MI', '3CON.MI', ema_period=50, period='2y', candles=0)
json_data=ts.download_json()
print(json_data)


ts = GenerateScoringAndSignals('3LPA.MI', '3LPA.MI', ema_period=50, period='2y', candles=0)
json_data=ts.download_json()
print(json_data)

db.save_json_data(json_data, nation, instrument_type)
ticker_data_df = db.get_sorted_tickers_by_keys("IT", "ETF", "percent_change_2", "volume",
                                              ["volume", "signal6", "percent_change_2", "scoring"], min_volume=1500)
print(ticker_data_df.to_string())