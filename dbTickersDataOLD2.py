...
import pandas as pd
class dbTickersData:
    def __init__(self, db_path="/home/developer/PycharmProjects/learningProjects/IFinance/stock_data4.db"):
    ...
    def get_sorted_tickers_by_signal6(self, nation, instrument_type, secondary_key='scoring', additional_columns=None):
        ...
        columns_to_select = ['ticker',  'signal6', secondary_key] + additional_columns
        ...
        df = pd.DataFrame(results, columns=['Ticker', 'Signal6', secondary_key] + additional_columns)
    ...
    def save_json_data(self, json_data_str, nation, instrument_type):
        ...
        # Ensure that 'Percent_Change_2' is converted to a float, and handle invalid or missing values
        try:
            percent_change_2 = float(signals['Percent_Change_2']) if signals['Percent_Change_2'] is not None else None
        except (ValueError, TypeError):
            percent_change_2 = None  # or set to a default value like 0.0

        values = [
            ticker, ticker_name, nation, instrument_type, signals['Open'], signals['High'], signals['Low'],
            signals['Close'], signals['Adj Close'], signals['Volume'], percent_change_2 ,
